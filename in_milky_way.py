...
Example:
    python in_milky_way.py 18.6156 38.7837
...
def _unwrap_lon(lon_deg: float, query_lon_deg: float) -> float:
    """Longitude relative to the query, wrapped to [-180, 180)."""
    return ((lon_deg - query_lon_deg + 180.0) % 360.0) - 180.0
...
def _ring_contains(ring: Ring, query_lon_deg: float, query_dec_deg: float) -> bool:
    """Boundary-inclusive planar point-in-ring test with seam-safe longitudes."""
    pts: list[Point] = [(_unwrap_lon(float(lon), query_lon_deg), float(dec)) for lon, dec in ring]
    if len(pts) < 3:
        return False

    px, py = 0.0, query_dec_deg
    inside = False
...
def _polygon_contains(rings: Sequence[Ring], lon_deg: float, dec_deg: float) -> bool:
    if not rings or not _ring_contains(rings[0], lon_deg, dec_deg):
        return False
    # GeoJSON rings after the first are holes. Their boundaries are still part
    # of the stored boundary, but their interiors are outside the region.
