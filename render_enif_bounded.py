...
def draw_enif_m15_objects(ax,spec,idx,center):
    for d in spec.get("deep_sky_objects",[]):
        ...
        if not xy:continue
        ax.scatter([xy[0]],[xy[1]],s=130,facecolors="none",edgecolors=r.ASTERISM_GREEN,linewidths=2.2,zorder=8)
        ...
        ref=d.get("from_ref")
        if not ref or ref not in idx:continue
        ...
        arrow=dict(arrowstyle="-|>",mutation_scale=22,linewidth=2.6,color=r.TEXT,shrinkA=0,shrinkB=18)
        ...
        if d.get("distance_label"):
            ax.annotate(d["distance_label"],joint,xytext=(0,9),textcoords="offset points",ha="center",va="bottom",fontsize=8,color=r.TEXT,bbox=dict(facecolor=r.NIGHT,edgecolor="none",pad=.5),zorder=9)
...
def make_great_square_green(svg_path:Path):
    """Postprocess only the Great Square stroke and label from figure blue to asterism green."""
    text=svg_path.read_text(encoding="utf-8")
    marker="Great Square of Pegasus"
    pos=text.find(marker)
    if pos<0:raise RuntimeError("Great Square label not found in Enif SVG")
    # Matplotlib emits the asterism path immediately before its text group.
    before=text[:pos]
    blue="#5c8fe8";green="#59c86d"
    pathpos=before.rfind(blue)
    if pathpos<0:raise RuntimeError("Great Square path color not found")
    before=before[:pathpos]+before[pathpos:].replace(blue,green,1)
    after=text[pos:]
    # Recolor the label's first blue occurrence too.
    after=after.replace(blue,green,1)
